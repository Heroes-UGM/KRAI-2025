...
void loop() {
  if (PS4.isConnected()) {
    ...
    // Map joystick values
    float rawVx = map(PS4.LStickX(), -128, 127, -255, 255) / 3.0;
    float rawVy = map(PS4.LStickY(), -128, 127, -255, 255) / 3.0;
    ...
    // Debug info
    Serial.printf("Vx: %.2f\tVy: %.2f\tw: %.2f   \t||\tV1: %.2f\tV2: %.2f\tV3: %.2f\tV4: %.2f\tInverse: %s\t", 
                   Vx, Vy, w, v1, v2, v3, v4, inverse ? "ON" : "OFF");
